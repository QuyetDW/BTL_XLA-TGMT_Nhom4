...
# Hàm xử lý ảnh với Decision Tree
def process_image_with_decision_tree(image_path, n_colors=5):
    ...
    # Sử dụng KMeans để tìm các màu sắc chính và tạo nhãn ban đầu
    kmeans = KMeans(n_clusters=n_colors, random_state=42)
    labels = kmeans.fit_predict(pixels)
    dominant_colors = kmeans.cluster_centers_.astype(int)

    # Huấn luyện Decision Tree với dữ liệu từ KMeans
    dtree = DecisionTreeClassifier(random_state=42)
    dtree.fit(pixels, labels)

    # Dự đoán nhãn mới cho toàn bộ pixel bằng Decision Tree
    predictions = dtree.predict(pixels)
    label_reshaped = predictions.reshape(50, 50)  # Chuyển thành ma trận
...
def evaluate_clustering(ground_truth, predictions):
    print("=== Đánh giá hiệu suất Decision Tree ===")
...
if __name__ == "__main__":
    ...
    if image_path:
        # Xử lý ảnh với Decision Tree
        image, dominant_colors, label_reshaped = process_image_with_decision_tree(image_path)
