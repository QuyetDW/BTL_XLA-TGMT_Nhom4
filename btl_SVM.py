...
def process_image_with_svm(image_path, n_colors=5):
    ...
    # KMeans để tìm màu chủ đạo
    kmeans = KMeans(n_clusters=n_colors, random_state=42)
    labels = kmeans.fit_predict(pixels)
    dominant_colors = kmeans.cluster_centers_.astype(int)

    # Huấn luyện SVM với dữ liệu từ KMeans
    svm = SVC(kernel='linear', random_state=42)
    svm.fit(pixels, labels)

    # Dự đoán các nhãn bằng SVM
    predictions = svm.predict(pixels)
    label_reshaped = predictions.reshape(50, 50)  # Đưa nhãn về dạng ma trận

    return image, dominant_colors, label_reshaped

# Hiển thị các cụm màu trên nền ảnh gốc
...
if __name__ == "__main__":
    ...
    if image_path:
        image, dominant_colors, label_reshaped = process_image_with_svm(image_path)

        # Giả lập nhãn gốc để đánh giá
